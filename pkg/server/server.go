...
type ProxyServer struct {
	...
	forwardConnectionManager *connectionManager
	nextConnID               int64
}
...
func NewProxyServer(serverID string, proxyStrategies []ProxyStrategy, serverCount int, agentAuthenticationOptions *AgentTokenAuthenticationOptions, warnOnChannelLimit bool) *ProxyServer {
	...
	return &ProxyServer{
		...
		nextConnID:               1,
		forwardConnectionManager: newConnectionManager(),
	}
}
...
func (s *ProxyServer) serveRecvBackend(backend Backend, stream agent.AgentService_ConnectServer, agentID string, recvCh <-chan *client.Packet) {
	defer func() {
		...
		s.forwardConnectionManager.Cleanup()
	}()
	...
	for pkt := range recvCh {
		...
		case client.PacketType_CLOSE_REQ:
			...
			kasConnCtx, agentConnection := s.forwardConnectionManager.Get(resp.ConnectID)
			if !agentConnection {
...
func (s *ProxyServer) handleDialRequest(pkt *client.Packet, backend Backend) {
	...
	s.forwardConnectionManager.Add(connID, kasConnCtx)
